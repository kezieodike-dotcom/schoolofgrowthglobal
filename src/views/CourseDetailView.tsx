...
export const CourseDetailView: React.FC<CourseDetailViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      {/* Hero Header */}
      <section className="relative overflow-hidden pt-12 pb-16 bg-gradient-to-b from-slate-900 to-slate-950 border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-amber-400 mb-4">
            ...
            <span>/</span>
            <span className="text-white font-bold">{FEATURED_COURSE.title}</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-6">
              ...
              {/* Key metadata chips */}
              <div className="flex flex-wrap items-center gap-6 pt-2 text-xs text-slate-300 font-mono">
              ...
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 flex items-center gap-4">
                <img src={FEATURED_COURSE.instructorAvatar} alt={FEATURED_COURSE.instructorName} className="w-12 h-12 rounded-full object-cover border-2 border-amber-500/40" />
                <div>
                  <h4 className="text-sm font-bold text-white font-serif">{FEATURED_COURSE.instructorName}</h4>
                  <p className="text-xs text-slate-400">{FEATURED_COURSE.instructorRole}</p>
                </div>
              </div>
            ...
            <div className="lg:col-span-4 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6 sticky top-28">
              ...
              <div className="space-y-1">
                <span className="text-xs text-slate-400 font-mono">Tuition & Sponsorship</span>
                <div className="text-2xl font-bold font-serif text-amber-400">{FEATURED_COURSE.price}</div>
              </div>
              ...
              <div className="space-y-3 pt-2">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-slate-950 hover:bg-slate-800 text-slate-300 border border-slate-800 font-semibold text-xs flex items-center justify-center gap-2 transition-colors"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {FEATURED_COURSE.outcomes.map((outcome, idx) => (
                    <div key={idx} className="p-4 rounded-xl bg-slate-900 border border-slate-800/80 flex items-start gap-3">
                  ...
                  {FEATURED_COURSE.modules.map((module, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div className="space-y-1">
                            <span className="text-xs font-mono text-amber-400 font-bold">{module.week}</span>
                            <h4 className="text-lg font-serif font-bold text-white">{module.title}</h4>
                          </div>
                          {isOpen ? <ChevronUp className="w-5 h-5 text-amber-400" /> : <ChevronDown className="w-5 h-5 text-slate-400" />}
                        </button>

                        {isOpen && (
                          <div className="px-6 pb-6 pt-2 border-t border-slate-800 space-y-4 text-xs text-slate-300">
                            <p className="leading-relaxed">{module.description}</p>
                            <div>
                              <span className="text-white font-bold block mb-2 font-mono">Module Focus Topics:</span>
              ...
              <div className="p-8 rounded-2xl bg-slate-900/60 border border-slate-800 text-center space-y-4">
                ...
                <div className="flex flex-wrap justify-center gap-8 text-slate-300 font-serif font-bold text-sm">
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="p-6 rounded-2xl bg-slate-900 border border-amber-500/30 space-y-4">
                ...
                {aiSyllabusPlan && (
                  <div className="p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-300 whitespace-pre-line leading-relaxed animate-fadeIn">
                    {aiSyllabusPlan}
                  </div>
                )}
              </div>
      ...
      {applyModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 text-slate-100 space-y-4 shadow-2xl relative">
            <button onClick={() => setApplyModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white">✕</button>

            <div className="space-y-1">
              <span className="text-xs font-mono text-amber-400 font-bold uppercase">Oct 15 COHORT APPLICATION</span>
              <h3 className="text-xl font-serif font-bold text-white">Apply for Executive Strategy & Global Growth</h3>
            </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); alert("Application submitted! Our admissions director will review your background within 24 hours."); setApplyModalOpen(false); }} className="space-y-3 text-xs">
              <div>
                ...
                <input required type="text" defaultValue="Chidi Okeke" className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white" />
              </div>
              <div>
                <label className="block text-slate-400 mb-1">Current Title & Organization</label>
                <input required type="text" defaultValue="VP of Global Strategy, Nexus International" className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white" />
              </div>
              <div>
                <label className="block text-slate-400 mb-1">Work Email</label>
                <input required type="email" defaultValue="alex.rivera@nexus.com" className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white" />
              </div>
