  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-4 text-center px-4">
        <h1 className="text-2xl font-serif font-bold text-white">Article not found</h1>
        <Link to="/blog" className="text-amber-400 hover:text-amber-300 text-sm flex items-center gap-1">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <Link to="/blog" className="inline-flex items-center gap-1.5 text-xs font-mono text-slate-400 hover:text-amber-400 mb-6">
        ...
        <h1 className="mt-4 text-3xl sm:text-4xl font-serif font-bold text-white leading-tight">{post.title}</h1>

        <div className="flex items-center gap-3 mt-5 text-xs text-slate-400 font-mono">
        ...
        <img src={post.image} alt={post.title} className="w-full h-72 object-cover rounded-2xl border border-slate-800 mt-8" />

        <div className="prose prose-invert mt-8 space-y-5 text-slate-300 text-sm leading-relaxed">
          <p className="text-base text-slate-200">{post.excerpt}</p>
          <p>
          ...
          <blockquote className="border-l-2 border-amber-500 pl-4 italic text-slate-300">
        ...
        <div className="mt-10 p-6 rounded-2xl bg-slate-900 border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-slate-300">Want the applied version, with tools and mentorship?</p>
        ...
        {/* Related */}
        <h3 className="mt-14 mb-6 text-xl font-serif font-bold text-white">Related Insights</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {related.map((r) => (
            <Link key={r.id} to={`/blog/${r.slug}`} className="group">
              <img src={r.image} alt={r.title} className="w-full h-28 object-cover rounded-xl border border-slate-800 opacity-80" />
              <span className="mt-2 block text-[10px] font-mono text-amber-400">{r.category}</span>
              <h4 className="text-sm font-serif font-bold text-white group-hover:text-amber-300 transition-colors leading-snug">
                {r.title}
              </h4>
            </Link>
          ))}
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Link
          ...
          className="group grid grid-cols-1 lg:grid-cols-2 gap-8 items-center mb-14 p-6 rounded-3xl bg-slate-900/70 border border-slate-800 hover:border-amber-500/40 transition-all"
        >
          <img src={featured.image} alt={featured.title} className="w-full h-64 object-cover rounded-2xl border border-slate-800 opacity-90" />
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 text-xs text-slate-400 font-mono pt-2">
              <span className="text-slate-200">{featured.author}</span>
              <span>•</span>
          ...
          {posts.map((post) => (
            ...
            >
              <div className="h-40 overflow-hidden">
                <img src={post.image} alt={post.title} className="w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500" />
              </div>
              <div className="p-5 flex flex-col flex-1">
                <span className="text-[10px] font-mono text-amber-400 flex items-center gap-1 mb-2">
                  <Tag className="w-3 h-3" /> {post.category}
                </span>
                <h4 className="text-lg font-serif font-bold text-white group-hover:text-amber-300 transition-colors mb-2 leading-snug">
                  {post.title}
                </h4>
                <p className="text-xs text-slate-400 line-clamp-2 mb-4 flex-1">{post.excerpt}</p>
