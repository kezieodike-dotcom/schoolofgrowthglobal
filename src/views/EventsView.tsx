  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <section className="py-14 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Type filter */}
        <div className="flex flex-wrap items-center gap-2 bg-slate-900 p-1.5 rounded-xl border border-slate-800 text-xs font-medium mb-10 w-fit">
          {TYPES.map((t) => (
            <button
              key={t}
              onClick={() => setFilter(t)}
              className={`px-3.5 py-2 rounded-lg transition-all ${
                filter === t ? 'bg-amber-500 text-slate-950 font-bold shadow' : 'text-slate-400 hover:text-white'
              }`}
            >
              {t}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((event) => (
            <div key={event.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col hover:border-slate-700 transition-all">
              ...
              <div className="p-5 flex flex-col flex-1">
                <h4 className="text-lg font-serif font-bold text-white mb-2">{event.title}</h4>
                <p className="text-xs text-slate-400 line-clamp-2 mb-4 flex-1">{event.description}</p>

                <div className="space-y-1.5 text-[11px] text-slate-400 font-mono mb-4">
                  <div className="flex items-center gap-1.5"><CalendarDays className="w-3.5 h-3.5 text-amber-400" /> {event.date} • {event.time}</div>
                  <div className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-amber-400" /> {event.location}</div>
                  <div className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-amber-400" /> {event.speaker}</div>
                </div>

                <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
                  <div>
                    <span className="text-sm font-bold font-serif text-amber-400">{event.price}</span>
                    <span className="block text-[10px] text-emerald-400 font-mono">{event.seatsLeft} seats left</span>
                  </div>
                  <button
      ...
      {registered && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-8 text-center space-y-4 shadow-2xl">
            ...
            <h3 className="text-lg font-serif font-bold text-white">You're Registered</h3>
            <p className="text-sm text-slate-400">
              A QR ticket and calendar invite for{' '}
              <span className="text-amber-400 font-medium">{EVENTS.find((e) => e.id === registered)?.title}</span> are on
